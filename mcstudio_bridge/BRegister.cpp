...
if(status!="Stopped"){
 ...
 if(stopMethod){stopMethod->Invoke(srv,nullptr);Thread::Sleep(1000);}
...
if(!File::Exists(dllPath)){L("ERR: "+dllPath+" not found");return;}
...
if(startMethod){startMethod->Invoke(srv,nullptr);Thread::Sleep(1000);}
status=statusProp->GetValue(srv)->ToString();
